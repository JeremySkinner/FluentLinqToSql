namespace FluentLinqToSql.Tests.ActiveRecord {
	...
	public class NameValueDeserializerTester {
		...
		public void EmptyPrefixThrows() {
			...
			typeof(ArgumentException).ShouldBeThrownBy(() => nvd.Deserialize(collection, string.Empty, typeof(Customer)));
		...
		public void NullTargetTypeThrows() {
			collection["junk"] = "stuff";
			typeof(ArgumentNullException).ShouldBeThrownBy(() => nvd.Deserialize(collection, "junk", null));
		}
	...
	public static class NVDExtensions {

		internal static T Deserialize<T>(this NameValueDeserializer nvd, NameValueCollection collection, string prefix) where T : new() {
			return (T)nvd.Deserialize(collection, prefix, typeof(T));
		}
